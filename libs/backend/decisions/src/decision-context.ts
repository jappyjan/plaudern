...
import type { DecisionExtractionInput, DecisionSpeaker } from './decisions.provider';
...
/**
 * Assembles the decision-extraction input for an item from its append-only
 ...
 * the model can attribute who was involved in a decision. Reads speaker
 * occurrences directly (like the summary/questions context) so the decisions
 * step stays independent of @plaudern/speaker-id and @plaudern/summarization.
 * Returns null when there is no succeeded transcription to extract from.
 */
@Injectable()
export class DecisionContextService {
  ...
  async build(
    ...
    // entirely, so a decision is never attributed to someone who withdrew.
    ...
    const speakers: DecisionSpeaker[] = roster.map((s, index) => ({
      label: s.label,
 ...
 * LABEL so the model can attribute participants; otherwise fall back to the
 * plain transcript text. Mirrors the summarization/questions context builder.
 */
