...
export class SettingsController {
  ...
  @Get()
  async get(): Promise<PlaudSettingsDto> {
    return this.settings.toDto(await this.settings.getEntity(), this.sync.isRunning);
  }

  @Put()
  async update(@Body() body: unknown): Promise<PlaudSettingsDto> {
    const req = updatePlaudSettingsRequestSchema.parse(body);
    const entity = await this.settings.upsert(req);
    if (entity.enabled) {
  ...
  @Post('test')
  async test(@Body() body: unknown): Promise<PlaudTestConnectionResponse> {
    const req = plaudTestConnectionRequestSchema.parse(body ?? {});
    try {
      const stored = await this.settings.getEntity();
